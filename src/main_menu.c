...
static void NewGameOliveSpeech_ShowDialogueWindow(u8, u8);
...
static void NewGameOliveSpeech_SetDefaultPlayerName(u8);
...
static void NewGameOliveSpeech_CreateDialogueWindowBorder(u8, u8, u8, u8, u8, u8);
...
static const u32 sOliveSpeechShadowGfx[] = INCGFX_U32("graphics/olive_speech/shadow.png", ".4bpp.lz");
static const u32 sOliveSpeechBgMap[] = INCGFX_U32("graphics/olive_speech/map.bin", ".lz");
...
static void Task_NewGameOliveSpeech_Init(u8 taskId)
{
    ...
    LZ77UnCompVram(sOliveSpeechShadowGfx, (void *)VRAM);
    LZ77UnCompVram(sOliveSpeechBgMap, (void *)(BG_SCREEN_ADDR(7)));
...
static void Task_NewGameOliveSpeech_WaitForSpriteFadeInWelcome(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        else
        {
            ...
            NewGameOliveSpeech_ShowDialogueWindow(0, 1);
            PutWindowTilemap(0);
            CopyWindowToVram(0, COPYWIN_GFX);
...
{
    int gender = NewGameOliveSpeech_ProcessGenderMenuInput();
    int gender2;
...
{
    return CreateMonPicSprite_Affine(SPECIES_MONOBERUS, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void NewGameOliveSpeech_SetDefaultPlayerName(u8 nameId)
{
    const u8 *name;
    u8 i;
...
static void NewGameOliveSpeech_ShowDialogueWindow(u8 windowId, u8 copyToVram)
{
    CallWindowFunction(windowId, NewGameOliveSpeech_CreateDialogueWindowBorder);
    FillWindowPixelBuffer(windowId, PIXEL_FILL(1));
...
static void NewGameOliveSpeech_CreateDialogueWindowBorder(u8 bg, u8 x, u8 y, u8 width, u8 height, u8 palNum)
{
    FillBgTilemapBufferRect(bg, OLIVE_DLG_BASE_TILE_NUM +  1, x-2,       y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, OLIVE_DLG_BASE_TILE_NUM +  3, x-1,       y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, OLIVE_DLG_BASE_TILE_NUM +  4, x,         y-1, width,   1, palNum);
    FillBgTilemapBufferRect(bg, OLIVE_DLG_BASE_TILE_NUM +  5, x+width-1, y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, OLIVE_DLG_BASE_TILE_NUM +  6, x+width,   y-1, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, OLIVE_DLG_BASE_TILE_NUM +  7, x-2,       y,   1,       5, palNum);
    FillBgTilemapBufferRect(bg, OLIVE_DLG_BASE_TILE_NUM +  9, x-1,       y,   width+1, 5, palNum);
    FillBgTilemapBufferRect(bg, OLIVE_DLG_BASE_TILE_NUM + 10, x+width,   y,   1,       5, palNum);

    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(OLIVE_DLG_BASE_TILE_NUM + 1), x-2,       y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(OLIVE_DLG_BASE_TILE_NUM + 3), x-1,       y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(OLIVE_DLG_BASE_TILE_NUM + 4), x,         y+height, width-1, 1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(OLIVE_DLG_BASE_TILE_NUM + 5), x+width-1, y+height, 1,       1, palNum);
    FillBgTilemapBufferRect(bg, BG_TILE_V_FLIP(OLIVE_DLG_BASE_TILE_NUM + 6), x+width,   y+height, 1,       1, palNum);
}
